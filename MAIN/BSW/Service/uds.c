...
/* SWC 변수/함수들 접근용 외부 선언 (실제 프로젝트에서는 swc.h 인클루드) */
extern uint8_t SWC_Get_ActEcu_CommStatus(void);   // 0: 정상, 1: 두절
extern uint8_t SWC_Get_BodyEcu_CommStatus(void);  // 0: 정상, 1: 두절
extern uint8_t SWC_Get_Pc_CommStatus(void);       // 0: 정상, 1: 두절

extern uint8_t SWC_Get_DtcStatus_ActEcu(void);    // DTC 상태 (1: 발생, 0: 정상)
extern uint8_t SWC_Get_DtcStatus_BodyEcu(void);   // DTC 상태
extern uint8_t SWC_Get_DtcStatus_Pc(void);        // DTC 상태

extern void SWC_Clear_All_DTC(void);              // 고장 코드(RAM 변수) 초기화 함수
...
void Uds_ProcessRx(uint8_t* reqData, uint16_t reqLength)
{
    ...
    switch (sid)
    {
        ...
        case 0x22:
        {
            ...
            {
                txBuf[txLen++] = SWC_Get_ActEcu_CommStatus();
            }
            else if (did == 0x0401) // Body ECU 통신 상태
            {
                txBuf[txLen++] = SWC_Get_BodyEcu_CommStatus();
            }
            else if (did == 0x0402) // PC 통신 상태
            {
                txBuf[txLen++] = SWC_Get_Pc_CommStatus();
            }
            else
            {
        ...
        case 0x19:
        {
            if (reqLength != 2) { Send_NegativeResponse(sid, 0x13); return; } // 0x0A는 마스크 파라미터가 없으므로 길이 2
            ...
            if (subFunc == 0x0A)
            {
                ...
                dtcStatus = (SWC_Get_DtcStatus_ActEcu() == 1) ? 0x09 : 0x00; // 발생: 0x09, 미발생(정상): 0x00
                txBuf[txLen++] = 0xC4; txBuf[txLen++] = 0x00; txBuf[txLen++] = 0x00;
                txBuf[txLen++] = dtcStatus;

                // 2. Body ECU 통신 두절 (0xC40100)
                dtcStatus = (SWC_Get_DtcStatus_BodyEcu() == 1) ? 0x09 : 0x00;
                txBuf[txLen++] = 0xC4; txBuf[txLen++] = 0x01; txBuf[txLen++] = 0x00;
                txBuf[txLen++] = dtcStatus;

                // 3. PC 통신 두절 (0xC40200)
                dtcStatus = (SWC_Get_DtcStatus_Pc() == 1) ? 0x09 : 0x00;
